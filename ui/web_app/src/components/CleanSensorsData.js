...
function CleanSensorsData() {
  ...
  const handleClean = async (e) => {
    e.preventDefault();
    setStatus("جاري رفع وتنظيف بيانات المستشعرات...");
    const formData = new FormData();
    formData.append("sensor_data", sensorFile);
    const res = await fetch("http://127.0.0.1:5000/clean-sensors-data", {
      method: "POST",
      body: formData
    });
  ...
  return (
    <div className="main-card">
      <div className="section-title" style={{textAlign: "center"}}>
        🛰️ تنظيف بيانات المستشعرات
      </div>
      <form onSubmit={handleClean} style={{direction: "rtl", textAlign: "right"}}>
        <div style={{marginBottom: 16}}>
          <label>رفع ملف بيانات المستشعرات (CSV):</label>
          <input
            type="file"
            accept=".csv"
        ...
        <div style={{textAlign: "center", marginTop: 24}}>
          <button type="submit" style={{
            ...
            padding: "10px 32px",
            fontSize: "1.1rem"
          }}>تنظيف بيانات المستشعرات</button>
        </div>
      </form>
...
export default CleanSensorsData;
